...
app.get('/', async (c) => {
  ...
  try {
    ...
    const aiSuggestions = generateAISuggestions(searchQuery, results as Array<{ title: string; category_name: string; potato_type_name: string }>);
...
app.get('/suggest', async (c) => {
  ...
  try {
    ...
    const { results: titleSuggestions } = await db.prepare(`
    ...
    const { results: ingredientSuggestions } = await db.prepare(`
    ...
    const { results: categorySuggestions } = await db.prepare(`
...
function generateAISuggestions(query: string, results: Array<{ title: string; category_name: string; potato_type_name: string }>) {
  const lowerQuery = query.toLowerCase();
  const suggestions: string[] = [];
  
  // Simple pattern matching for suggestions
  if (lowerQuery.includes('quick') || lowerQuery.includes('fast')) {
    suggestions.push('Try filtering for recipes under 30 minutes using the max_time parameter');
  }
  
  if (lowerQuery.includes('easy') || lowerQuery.includes('simple')) {
    suggestions.push('Most of our recipes are designed to be kid-friendly with difficulty level "easy"');
  }
  
  if (lowerQuery.includes('healthy') || lowerQuery.includes('nutritious')) {
    suggestions.push('Sweet potatoes are packed with vitamins! Try recipes featuring sweet potatoes.');
  }
  
  if (results.length === 0) {
    suggestions.push('No exact matches found. Try searching for ingredient names like "cheese", "butter", or "salt"');
    suggestions.push('Or browse all recipes by category using /api/recipes/meta/categories');
  } else if (results.length < 5) {
    suggestions.push(`Found ${results.length} result${results.length === 1 ? '' : 's'}. Try broadening your search or browse by potato type.`);
  }
  
  // Ingredient-based suggestions
  const ingredientMatches = lowerQuery.match(/\b(potato|cheese|butter|milk|egg|salt|pepper|oil)\b/g);
  if (ingredientMatches) {
    suggestions.push(`You searched for ${ingredientMatches.join(', ')}. Check the alternative ingredients section in recipes for substitution ideas!`);
  }
  ...
  return {
    ...
    potential_filters: {
      ...
      potato_types: results.slice(0, 3).map(r => r.potato_type_name).filter(Boolean),
    },
  };
}
